...
public class bai3 {
    ...
    public static void main(String[] args) {
        ...
        for (int i=0;i<n;i++){
            System.out.println("Nhap so dien lan thu: " + (i + 1));
            double soDien = sc.nextDouble();
            double soTien = tinhTienDien(soDien);
            System.out.println("So tien dien la: " + (i + 1) + ":" + soTien);
        }
        ...
        do {
            ...
            System.out.println("So tien dien la: " + (i + 1) + ":" + soTien);
            i++;
            System.out.println("Nhap vao 1 de tiep tuc, 0 de thoat ");
            thoat = sc.nextInt();
        }while (thoat == 1); // kiểm tra điều kiện dừng, nếu điều kiện = true => tiếp tục lặp, ngược lại = false => thoát vòng lặp

        // while : kiểm tra điều kiện lặp trước, nếu thỏa mãn suy ra thực hiện khối lệnh, nếu không thỏa mãn => không thực hiện khối lệnh
        */
        int j = 0;
        int thoatWhile = 1;
        while (thoatWhile == 1){
