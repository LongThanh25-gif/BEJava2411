...
create table student ( -- định nghĩa tên bảng, tạo bảng
                         id integer primary key auto_increment, -- tạo ra các cột với cấu trúc (ten_cot kiểu_dữ_liệu)
                         full_name nvarchar(50), -- nvarchar lưu tên tiếng việt
                         date_of_birth date, -- data lưu trữ kiểu dữ liệu date
    ...
    -- avatar blob, -- lưu trữ dạng file
                         point float -- lữu trữ số lẻ
);
-- khóa chính, giúp xác định tính duy nhất độc lập trong bảng, vì hai bảng ghi trong cùng 1 table không được trùng khóa chính với nhau
...
(full_name, date_of_birth, code, deleted , description, point) -- các cột muốn thêm dữ liệu
    ...
    ('Nguyễn Thành Long', '2021-01-01','S001',0,'Sinh viên quê tại HP',7.5);
insert into -- câu lệnh thêm dữ liệu
    student -- tên table muốn thêm
(full_name, date_of_birth, code, deleted, description, point) -- các cột muốn thêm dữ liệu
    value -- từ khóa chỉ định đây là các value của cột
...
/*
 ...
 1) tạo table lớp học với các thông tin
    1) id là khóa chính,
    2) code lưu mã của lớp học
    3) ngày mở lớp
    4) deleted lưu true, false lưu trạng thái lớp bị xóa chưa
